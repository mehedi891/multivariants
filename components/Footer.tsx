...
export default function Footer() {
  return (
    <footer className="bg-brand-dark text-white/75 px-[5%] pt-16 pb-8">
                ...
                {[{ href: "#", label: "Facebook", abbr: "FB" }, { href: "#", label: "Instagram", abbr: "IN" }, { href: "#", label: "Twitter", abbr: "TW" }].map((s) => (
                  <li key={s.abbr}>
                    <a
                      href={s.href}
                      aria-label={s.label}
                      className="w-9 h-9 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center text-white/60 text-[13px] font-bold hover:bg-primary hover:border-primary hover:text-white transition-all"
                    >
                      {s.abbr}
                    </a>
                  </li>
